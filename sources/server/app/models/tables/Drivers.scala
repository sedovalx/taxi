...
class Drivers(tag: Tag) extends TableBase[Driver](tag, "driver") {
  ...
  def * = (id, pass, license, lastName, firstName, middleName, phone, secPhone, address, creationDate, editDate, creatorId, editorId, comment) <> (Driver.tupled, Driver.unapply)
